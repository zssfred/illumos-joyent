 ...
 *
 * XXX: same accross archs
 */
int
kobj_addrcheck(void *xmp, caddr_t addr)
{
	struct module *mp;

	mp = (struct module *)xmp;

	if ((addr >= mp->text && addr < mp->text + mp->text_size) ||
	    (addr >= mp->data && addr < mp->data + mp->data_size))
		return (0); /* ok */
	if (mp->bss && addr >= (caddr_t)mp->bss &&
	    addr < (caddr_t)mp->bss + mp->bss_size)
		return (0);
	return (1);
}

/*
 * Flush instruction cache after updating text. ARM has no synchronization
 * here so it's up to us to first ensure that the d-cache is written out and
 * then to flush that region of the i-cache.
 */
void
kobj_sync_instruction_memory(caddr_t addr, size_t len)
{
	arm_text_flush(addr, len);
}

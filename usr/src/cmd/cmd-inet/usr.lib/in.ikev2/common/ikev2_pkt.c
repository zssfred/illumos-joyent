    ...
    ikev2_xf_type_t xftype, uint16_t xfid, void *restrict cookie)
{
	...
	while (ptr < end) {
		...
		if (cb != NULL && !cb(xftype, xfid, &attr, cookie))
			return (B_TRUE);
...
/*
 * If nonce is NULL, create a nonce of len bytes and add to packet.  If
 * nonce != NULL, add the nonce of the given length to the packet.  If nonce
 * != NULL, it should be one previously generated by ikev2_add_nonce, but
 * we had to restart the IKE_SA_INIT exchange (due to cookies or needing a
 * new DH key).
 *
 * When generating a new nonce, the size is capped to reside within the
 * range [IKEV2_NONCE_MIN, IKEV2_NONCE_MAX].
 */
boolean_t
ikev2_add_nonce(pkt_t *restrict pkt, uint8_t *restrict nonce, size_t len)
{
	if (nonce != NULL) {
		/*
		 * If we already created the nonce, it should already be
		 * within the limits.
		 */
		VERIFY3U(len, <=, IKEV2_NONCE_MAX);
		VERIFY3U(len, >=, IKEV2_NONCE_MIN);
	} else {
		len = MIN(len, IKEV2_NONCE_MAX);
		len = MAX(len, IKEV2_NONCE_MIN);
	}
	...
	if (nonce != NULL) {
		 ...
		 * getentropy(3C) is limited to 256 bytes, which also happens
		 * to be IKEV2_NONCE_MAX, so a single call is all that is
		 * necessary.  If IKEV2_NONCE_MAX ever increases, the CTASSERT
		 * should trip on compile, and this bit of code will need to
		 * be adjusted to deal with potentially larger nonce sizes.
		 */
